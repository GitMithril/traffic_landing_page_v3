        ...
        <div className="relative mt-16 md:mt-20">
          ...
          <ol className="flex flex-col md:flex-row">
            {DISCIPLINES.map((d) => (
              <li
                key={d.name}
                className="relative flex-1 border-t border-[var(--color-line)] pl-8 pt-8 pb-2 first:border-t-0 md:border-t-0 md:border-l md:pl-8 md:pt-10 md:first:border-l-0 md:first:pl-0"
              >
                <span
                  aria-hidden
                  className="absolute left-0 top-8 hidden h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[var(--color-accent)] md:block md:top-0"
                />
                <span
                  aria-hidden
                  className="absolute left-0 top-8 h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[var(--color-accent)] md:hidden"
                />
                <h3 className="text-[1.4rem] font-bold tracking-[-0.01em] text-[var(--color-ink)]">
                  {d.name}
                </h3>
