...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-[var(--color-line-on-black)] bg-white/5 backdrop-blur-[16px] text-[var(--color-ink-on-black-muted)] cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.3)] hover:border-white/25 hover:text-[var(--color-ink-on-black)] active:opacity-70 transition-colors duration-300 before:content-[''] before:absolute before:inset-[3px] before:rounded-full before:border before:border-white/[0.04] before:pointer-events-none";
          ...
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden">
                {/* eslint-disable-next-line @next/next/no-img-element -- GSAP drives layout via imperative transforms on this exact element */}
                <img
                  src={card.imgUrl}
                  loading="lazy"
            ...
            {cards.map((_, i) => (
              <span
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  i === centerIndex ? "bg-[var(--color-accent)] scale-[1.3]" : "bg-white/15"
                }`}
              />
            ))}
          </div>
