...
### Hierarchy
- **Display** (500, `clamp(2.75rem, 6vw, 5.25rem)`, line-height 0.98, tracking `-1px` below the `md` breakpoint, `-3.5px` at `md` and up): closing CTA headline only.
- **Headline** (500, `clamp(2rem, 4vw, 3.25rem)`, line-height 1.04, same responsive tracking split): section headlines (problem, services, testimonials, pricing, qualification, booking explainer). Hero headline uses the same weight at a slightly tighter clamp (`clamp(2.5rem, 5.2vw, 4.25rem)`, 24ch max width) as the largest first-viewport instance. The `-3.5px` tracking is a flat pixel value (not em-scaled) — client-specified — which visibly collides letters at the small end of these `clamp()` ranges on mobile, so it only applies from `md` up; mobile uses a lighter `-1px` so the same headline stays legible at its smallest rendered size.
...
- **Reuse:** this is a system-level pattern, not a testimonial-only device. It appears in the testimonial quote cards, the qualification section's "who this is for" fit card, and the pricing section's per-tier portrait panel. All three share the same visual grammar: true-black ground, top-anchored gradient wash, scrim, centered or top-aligned content. The pricing portrait panel now shows a real piece of Doomsday's own creative work per tier (client-sourced from the project's media library, one image per plan, escalating in intensity from Essentials to Overdrive) instead of the brand-mark placeholder — the image sits under a low-opacity (`mix-blend-overlay`, ~35%) tier-colored gradient wash and a bottom scrim carrying the plan name, so it still reads as "the gradient-wash card family" rather than a bare photo. Source paths live in `src/lib/plan-portraits.ts` alongside each tier's wash colors.
